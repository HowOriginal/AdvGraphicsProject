...
glm::vec3 GLCanvas::TraceRay(double i, double j) {
  ...
  {
	  //Stratified sampling
	  int omega = ceil(sqrt(args->num_antialias_samples));
	  ...
	  double xO = (i - args->width / 2.0) / double(max_d) + 0.5;
	  double yO = (j - args->height / 2.0) / double(max_d) + 0.5;
	  ...
	  for (int n = 0; n < args->num_antialias_samples; n++)
	  {
		  ...
		  double x = xO;
		  double y = yO;
		  x += stratum[n].first * pixelsize + GLOBAL_MTRAND.rand() * pixelsize * omega;
		  y += stratum[n].second * pixelsize + GLOBAL_MTRAND.rand() * pixelsize * omega;
		  //End stratified code

...
*/
	  double x0 = (i - args->width / 2.0) / double(max_d) + 0.5;
	  double y0 = (j - args->height / 2.0) / double(max_d) + 0.5;
	  double pixelsize = .005;
	  x0 -= pixelsize / 2;
	  y0 -= pixelsize / 2;
	  ...
	  for (int m = 0; m < 2; m++)
	  {
		  for (int n = 0; n < 2; n++)
		  {
			  double x = x0 + 0.333 * (m+1) * pixelsize;
			  double y = y0 + 0.333 * (n+1) * pixelsize;
	  ...
	  for (int m = 0; m < 4; m++)
	  {
		  for (int n = m + 1; n < 4; n++)
		  {
			  if (abs(colors[m].r - colors[n].r) > VarianceLimit ||
				  ...
				  abs(colors[m].b - colors[n].b) > VarianceLimit)
			  {
				  ...
				  //Problem pixel
				  /*
				  double pixelsize = .001;
				  Color newcol;
				  double x0 = (i - args->width / 2.0) / double(max_d) + 0.5 - pixelsize/2;
				  double y0 = (j - args->height / 2.0) / double(max_d) + 0.5 - pixelsize/2;
				  for (int z = 0; z < args->num_antialias_samples; z++)
				  {
					  double x = x0 + pixelsize*GLOBAL_MTRAND.rand();
					  double y = y0 + pixelsize*GLOBAL_MTRAND.rand();
					  Ray r = camera->generateRay(x, y);
					  Hit hit;
					  glm::vec3 part = raytracer->TraceRay(r, hit, args->num_bounces);
...
void GLCanvas::WriteToFile()
{
	...
	for (int i = 0; i < args->width; i++)
	{
		for (int j = 0; j < args->height; j++)
		{
			...
			newfile.SetPixel(i, j, Color (r, g, b));
		}
	}
